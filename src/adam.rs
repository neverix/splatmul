...
const fn generate_dtq_lut(signed: bool, divisor: f32) -> [f32; 256] {
    let mut numbers: [f32; 256] = [0.0; 256];
            ...
            while remainder < if add_value > 0 { add_value * 2 } else { 1 } {
                ...
                let value: f32 = sign
                    ...
                    * (if digits > 0 {
                    ...
                    / (2_i32.pow(digits as u32) as f32);
...
fn simd_abs<const N: usize>(value: Simd<f32, N>) -> Simd<f32, N>
where
...
fn f32_to_dtq_simd<const N: usize, const SIGNED: bool>(value: f32) -> u8
where
    LaneCount<N>: SupportedLaneCount,
{
    let lut = if SIGNED {
    ...
    let best_candidate = (best_candidate
        + best_simd
            .simd_gt(best_distance_simd)
            .to_bitmask()
            .trailing_ones() as usize) as u8;
    best_candidate
}
...
impl<const SIGNED: bool> BlockScaled<SIGNED> {
    ...
    pub fn par_for_each(&mut self, visitor: impl Fn(&mut [u8], &mut [f32], usize) + Sync) {
        make_progress!(self
            .block
            .as_mut_slice()
            .par_chunks_mut(self.block_size * CHUNK_CHUNK)
            .zip(self.scales.par_chunks_mut(CHUNK_CHUNK))
            .enumerate()
            .into_par_iter())
        .for_each(|(i, (block_chunk, scale))| {
    ...
    pub fn par_f32_for_each(&mut self, visitor: impl Fn(&mut [f32], usize) + Sync) {
        ...
        self.par_for_each(|block_chunk, scale, i| {
            let chunk_array =
                ArrayViewMut2::from_shape((CHUNK_CHUNK, block_size), block_chunk).unwrap();
            ...
            scaled_chunk_array_f32
                .axis_iter_mut(ndarray::Axis(0))
                .enumerate()
                .for_each(|(j, mut array)| {
                    visitor(array.as_slice_mut().unwrap(), i + j * block_size);
                });
            let new_scale = scaled_chunk_array_f32.abs().map_axis(Axis(1), |x| {
                ADAM_FLOOR.max(
                    *x.into_iter()
    ...
    pub fn par_iter(&self) -> impl IndexedParallelIterator<Item = f32> + '_ {
        (0..self.block.len())
            .into_par_iter()
            .map(|i| dtq_to_f32::<SIGNED>(self.block[i]) * self.scales[i / self.block_size])
    }
}
